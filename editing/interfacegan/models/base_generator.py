...
def get_temp_logger(logger_name="logger"):
    ...
    """
    if not logger_name:
        raise ValueError("Input `logger_name` should not be empty!")
...
class BaseGenerator(object):
    ...
    def __init__(self, model_name, logger=None):
        ...
        if os.path.isfile(getattr(self, "model_path", "")):
            ...
            self.logger.warning("No pre-trained model will be loaded!")
    ...
    def check_attr(self, attr_name):
        ...
        if not hasattr(self, attr_name):
            raise AttributeError(f"`{attr_name}` is missing for model `{self.model_name}`!")
    ...
    def easy_synthesize(self, latent_codes, q=None, **kwargs):
        """Wraps functions `synthesize()` and `postprocess()` together."""
        outputs = self.synthesize(latent_codes, q=q, **kwargs)
        if "image" in outputs:
