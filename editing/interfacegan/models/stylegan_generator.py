...
class StyleGANGenerator(BaseGenerator):
    ...
    def convert_tf_model(self, test_num=10):
        ...
        tf_vars.update(dict(tf_model.components.mapping.__getstate__()["variables"]))
        tf_vars.update(dict(tf_model.components.synthesis.__getstate__()["variables"]))
        ...
        for i in range(test_num):
            ...
            tf_output = tf_model.run(
                latent_code,  # latents_in
                None,  # labels_in
                truncation_psi=self.truncation_psi,
    ...
    def synthesize(self, latent_codes, latent_space_type="Z", generate_style=False, generate_image=True):
        ...
        if generate_style:
            for i in range(self.num_layers):
                style = self.model.synthesis.__getattr__(f"layer{i}").epilogue.style_mod.dense(wps[:, i, :])
