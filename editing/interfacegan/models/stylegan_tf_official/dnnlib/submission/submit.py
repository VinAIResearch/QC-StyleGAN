...
def _populate_run_dir(run_dir: str, submit_config: SubmitConfig) -> None:
    ...
    files += [
        (os.path.join(dnnlib_module_dir_path, "submission", "_internal", "run.py"), os.path.join(run_dir, "run.py"))
    ]
...
def run_wrapper(submit_config: SubmitConfig) -> None:
    ...
    if is_local:
        logger = util.Logger(
            file_name=os.path.join(submit_config.run_dir, "log.txt"), file_mode="w", should_flush=True
        )
    ...
    try:
        ...
        util.call_func_by_name(
            func_name=submit_config.run_func_name, submit_config=submit_config, **submit_config.run_func_kwargs
        )
        print(
