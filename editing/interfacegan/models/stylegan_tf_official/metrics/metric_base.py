...
ppl_wfull = dnnlib.util.EasyDict(
    func_name="metrics.perceptual_path_length.PPL",
    name="ppl_wfull",
    num_samples=100000,
    epsilon=1e-4,
...
dummy = dnnlib.util.EasyDict(func_name="metrics.metric_base.DummyMetric", name="dummy")  # for debugging
...
class MetricBase:
    ...
    def _report_result(self, value, suffix="", fmt="%-10.4f"):
        self._results += [dnnlib.util.EasyDict(value=value, suffix=suffix, fmt=fmt)]

    def _get_cache_file_for_reals(self, extension="pkl", **kwargs):
        all_args = dnnlib.util.EasyDict(metric_name=self.name, mirror_augment=self._mirror_augment)
