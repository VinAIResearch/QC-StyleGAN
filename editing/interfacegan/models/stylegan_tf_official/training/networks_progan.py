...
def G_paper(
    ...
    **_kwargs
        ...
        with tf.variable_scope("%dx%d" % (2**res, 2**res)):
            if res == 2:  # 4x4
                ...
                with tf.variable_scope("Dense"):
                    x = dense(
                        x, fmaps=nf(res - 1) * 16, gain=np.sqrt(2) / 4, use_wscale=use_wscale
                ...
                if fused_scale:
                    with tf.variable_scope("Conv0_up"):
                        x = PN(
                            act(apply_bias(upscale2d_conv2d(x, fmaps=nf(res - 1), kernel=3, use_wscale=use_wscale)))
                        )
                else:
                    x = upscale2d(x)
    ...
    if structure == "recursive":
        ...
        def get_up(y, res, lod):
            return lambda: upscale2d(torgb(y, res), 2**lod)

        def grow(x, res, lod):
            y = block(x, res)
            img = get_up(y, res, lod)
            if res > 2:
                img = cset(
                    img,
                    (lod_in > lod),
                    lambda: upscale2d(lerp(torgb(y, res), upscale2d(torgb(x, res - 1)), lod_in - lod), 2**lod),
                )
            if lod > 0:
...
def D_paper(
    ...
    **_kwargs
    ...
    if structure == "recursive":
        ...
        def get_down(images_in, lod, res):
            return lambda: fromrgb(downscale2d(images_in, 2**lod), res)

        def get_v(x):
            return lambda: x

        def grow(res, lod):
            x = get_down(images_in, lod, res)
            if lod > 0:
