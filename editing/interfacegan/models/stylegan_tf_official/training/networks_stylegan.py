...
def G_style(
    ...
    components=dnnlib.util.EasyDict(),  # Container for sub-networks. Retained between calls.
    **kwargs
):  # Arguments for sub-networks (G_mapping and G_synthesis).

    # Validate arguments.
    assert not is_training or not is_validation
    assert isinstance(components, dnnlib.util.EasyDict)
    if is_validation:
...
def G_synthesis(
    ...
    **_kwargs
    ...
    if structure == "recursive":
        ...
        def get_up(res, y, lod):
            return lambda: upscale2d(torgb(res, y), 2**lod)
