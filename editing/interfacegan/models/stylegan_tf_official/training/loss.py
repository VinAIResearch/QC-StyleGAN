...
def D_wgan(
    G, D, opt, training_set, minibatch_size, reals, labels, wgan_epsilon=0.001  # pylint: disable=unused-argument
...
def D_hinge(G, D, opt, training_set, minibatch_size, reals, labels):  # pylint: disable=unused-argument
    ...
    real_scores_out = autosummary("Loss/scores/real", real_scores_out)
    fake_scores_out = autosummary("Loss/scores/fake", fake_scores_out)
    loss = tf.maximum(0.0, 1.0 + fake_scores_out) + tf.maximum(0.0, 1.0 - real_scores_out)
    return loss
...
def D_logistic(G, D, opt, training_set, minibatch_size, reals, labels):  # pylint: disable=unused-argument
    ...
    loss += tf.nn.softplus(
        -real_scores_out
    )  # -log(logistic(real_scores_out)) # temporary pylint workaround # pylint: disable=invalid-unary-operand-type
    return loss
