        ...
        with tf.name_scope(self.id + "_grad"), tf.device(dev):
            if dev not in self._dev_opt:
                ...
                self._dev_opt[dev] = self.optimizer_class(
                    name=opt_name, learning_rate=self.learning_rate, **self.optimizer_kwargs
                )
                self._dev_grads[dev] = []
            loss = self.apply_loss_scaling(tf.cast(loss, tf.float32))
            grads = self._dev_opt[dev].compute_gradients(
                loss, vars, gate_gradients=tf.train.Optimizer.GATE_NONE
                        ...
                        if not self.use_loss_scaling:
                            ...
                            ops.append(
                                tf.cond(
                                    grad_ok,
                                    lambda: tf.group(
                                        tf.assign_add(ls_var, self.loss_scaling_inc), opt.apply_gradients(grads)
                                    ),
        ...
        def recurse(scope, parent_ops, level):
            ...
            # Otherwise => interpret as a layer.
                ...
                layer_trainables = [
                    op.outputs[0]
                    for op in ops
                    if op.type.startswith("Variable") and self.get_var_localname(op.name) in self.trainables
                ]
