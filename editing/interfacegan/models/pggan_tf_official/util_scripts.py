...
def generate_fake_images(
    ...
    for png_idx in range(num_pngs):
        ...
        misc.save_image_grid(
            images, os.path.join(result_subdir, "%s%06d.png" % (png_prefix, png_idx)), [0, 255], grid_size
        )
...
def generate_interpolation_video(
    ...
    moviepy.editor.VideoClip(make_frame, duration=duration_sec).write_videofile(
        os.path.join(result_subdir, mp4), fps=mp4_fps, codec="libx264", bitrate=mp4_bitrate
    )
    open(os.path.join(result_subdir, "_done.txt"), "wt").close()
...
def generate_training_video(
    run_id, duration_sec=20.0, time_warp=1.5, mp4=None, mp4_fps=30, mp4_codec="libx265", mp4_bitrate="16M"
):
    ...
    def make_frame(t):
        ...
        img = misc.draw_text_label(
            img, misc.format_time(int(np.rint(wallclock))), img.shape[1] // 2, img.shape[0] - 4, alignx=0.5, aligny=1.0
        )
        img = misc.draw_text_label(
            img, "%.0f kimg" % kimg, img.shape[1] - 16, img.shape[0] - 4, alignx=1.0, aligny=1.0
        )
        return img
...
def evaluate_metrics(run_id, log, metrics, num_images, real_passes, minibatch_size=None):
    ...
    for name in metrics:
        ...
        obj = class_def(
            num_images=num_images, image_shape=image_shape, image_dtype=np.uint8, minibatch_size=minibatch_size
        )
