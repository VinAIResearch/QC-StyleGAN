...
def patch_theano_gan(state):
    ...
    assert not spec.pop("use_batchnorm", False)
    ...
    assert not spec.pop("use_layernorm", False)
    ...
    def he_std(gain, w):
        return gain / np.sqrt(np.prod(w.shape[:-1]))

    def wscale(gain, w):
        return w * next(param_iter) / he_std(gain, w) if use_wscale else w

    def layer(name, gain, w):
        return [(name + "/weight", wscale(gain, w)), (name + "/bias", next(param_iter))]
    ...
    if func.startswith("G"):
        ...
        for res in range(3, resolution_log2 + 1):
            vars += layer(
                "%dx%d/Conv0" % (2**res, 2**res), relu, next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1]
            )
            vars += layer(
                "%dx%d/Conv1" % (2**res, 2**res), relu, next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1]
            )
    ...
    if func.startswith("D"):
        ...
        for res in range(resolution_log2, 2, -1):
            ...
            vars += layer(
                "FromRGB_lod%d" % (resolution_log2 - (res - 1)), relu, next(param_iter)[np.newaxis, np.newaxis]
            )
        vars += layer("4x4/Conv", relu, next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1])
