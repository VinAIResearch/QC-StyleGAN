    ...
    def __init__(
        ...
        for pth_var_name, tf_var_name in _STYLEGAN_PTH_VARS_TO_TF_VARS.items():
            if "Conv0_up" in tf_var_name:
                ...
                if (self.fused_scale is True) or (self.fused_scale == "auto" and res >= _AUTO_FUSED_SCALE_MIN_RES):
...
class MappingModule(nn.Sequential):
    ...
    def __init__(
        self, normalize_input=True, input_space_dim=512, hidden_space_dim=512, final_space_dim=512, num_layers=8
    ):
...
class TruncationModule(nn.Module):
    ...
    def __init__(self, resolution=1024, w_space_dim=512, truncation_psi=0.7, truncation_layers=8):
        super().__init__()
...
class SynthesisModule(nn.Module):
    ...
    def __init__(self, resolution=1024, fused_scale="auto", output_channels=3, randomize_noise=False):
        super().__init__()
        ...
        for block_idx in range(1, len(self.channels)):
            if block_idx == 1:
                self.add_module(
                    ...
                    FirstConvBlock(in_channels=self.channels[block_idx - 1], randomize_noise=randomize_noise),
                )
            else:
                self.add_module(
                    f"layer{2 * block_idx - 2}",
            ...
            self.add_module(
                ...
                LastConvBlock(in_channels=self.channels[block_idx], out_channels=output_channels),
            )
...
class StyleModulationLayer(nn.Module):
    ...
    def forward(self, x, w):
        if len(w.shape) != 2:
            raise ValueError(
                f"The input tensor should be with shape [batch_size, " f"num_channels], but {x.shape} received!"
            )
        style = self.dense(w)
...
class WScaleLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, gain=np.sqrt(2.0), lr_multiplier=1.0):
        super().__init__()
...
class EpilogueBlock(nn.Module):
    ...
    def __init__(self, layer_idx, channels, randomize_noise=False, normalization_fn="instance"):
        super().__init__()
        ...
        if normalization_fn == "pixel":
            ...
            raise NotImplementedError(f"Not implemented normalization function: " f"{normalization_fn}!")
...
class FirstConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, randomize_noise=False):
        ...
        self.epilogue = EpilogueBlock(layer_idx=0, channels=in_channels, randomize_noise=randomize_noise)
...
class UpConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if layer_idx % 2 == 1:
            raise ValueError(
                f"This block is implemented as the first block of each "
                f"resolution, but is applied to layer {layer_idx}!"
            )
        if fused_scale not in [True, False, "auto"]:
            raise ValueError(f"`fused_scale` can only be [True, False, `auto`], " f"but {fused_scale} received!")
        ...
        self.epilogue = EpilogueBlock(layer_idx=layer_idx, channels=out_channels, randomize_noise=randomize_noise)
...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if layer_idx % 2 == 0:
            raise ValueError(
                f"This block is implemented as the second block of each "
                f"resolution, but is applied to layer {layer_idx}!"
            )
...
class DenseBlock(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.linear = nn.Linear(in_features=in_features, out_features=out_features, bias=add_bias)
