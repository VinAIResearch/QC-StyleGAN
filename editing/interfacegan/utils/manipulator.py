...
def train_boundary(
    ...
    if not isinstance(latent_codes, np.ndarray) or not len(latent_codes.shape) == 2:
        raise ValueError(
            "Input `latent_codes` should be with type" "`numpy.ndarray`, and shape [num_samples, " "latent_space_dim]!"
        )
    ...
    if (
        ...
        raise ValueError(
            "Input `scores` should be with type `numpy.ndarray`, and "
...
def project_boundary(primal, *args):
    ...
    if len(args) == 1:
        cond = args[0]
        assert len(cond.shape) == 2 and cond.shape[0] == 1 and cond.shape[1] == primal.shape[1]
        new = primal - primal.dot(cond.T) * cond
        return new / np.linalg.norm(new)
    elif len(args) == 2:
        cond_1 = args[0]
        cond_2 = args[1]
        assert len(cond_1.shape) == 2 and cond_1.shape[0] == 1 and cond_1.shape[1] == primal.shape[1]
        assert len(cond_2.shape) == 2 and cond_2.shape[0] == 1 and cond_2.shape[1] == primal.shape[1]
        primal_cond_1 = primal.dot(cond_1.T)
        primal_cond_2 = primal.dot(cond_2.T)
        cond_1_cond_2 = cond_1.dot(cond_2.T)
        alpha = (primal_cond_1 - primal_cond_2 * cond_1_cond_2) / (1 - cond_1_cond_2**2 + 1e-8)
        beta = (primal_cond_2 - primal_cond_1 * cond_1_cond_2) / (1 - cond_1_cond_2**2 + 1e-8)
        new = primal - alpha * cond_1 - beta * cond_2
        return new / np.linalg.norm(new)
    else:
        for cond_boundary in args:
            assert (
                len(cond_boundary.shape) == 2
                and cond_boundary.shape[0] == 1
                and cond_boundary.shape[1] == primal.shape[1]
            )
...
def linear_interpolate(latent_code, boundary, start_distance=-3.0, end_distance=3.0, steps=10):
    ...
    """
    assert (
        latent_code.shape[0] == 1
        and boundary.shape[0] == 1
