...
class ResNetGradualStyleEncoder(Module):
    ...
    This classes uses an FPN-based architecture applied over an ResNet34 backbone.
    """
...
class ResNetEncoderUsingLastLayerIntoW(Module):
    def __init__(self, dim):
        ...
        self.output_pool = torch.nn.AdaptiveAvgPool2d((1, 1))
        self.out_dim = dim
        self.linear = EqualLinear(512, self.out_dim, lr_mul=1)
...
class DoubleEncoder(Module):
    def __init__(self, q_dim):
        ...
        self.content_encoder = ResNetEncoderUsingLastLayerIntoW(512)

        if self.q_dim is not None:
            self.quality_encoder = ResNetEncoderUsingLastLayerIntoW(q_dim)

    def forward(self, x, skip_q):
        d = self.content_encoder(x)
        if self.q_dim is None:
            return d

        if skip_q:
            q = torch.zeros((d.shape[0], self.q_dim), device=d.device)
        else:
            q = self.quality_encoder(x)

        return torch.cat((d, q), dim=1)


# ---------------------------------------------------------------#

if __name__ == "__main__":
    model = ResNetGradualStyleEncoder()
