...
class SingleIDCoach(BaseCoach):
    ...
    def train(self):
        ...
        for fname, image, latent in tqdm(self.data_loader):
            ...
            org_sharp_images, _ = self.original_G.synthesis(
                w_pivot, torch.zeros_like(q, device=q.device), noise_mode="const", force_fp32=True
            )
            ...
            if self.gen_degraded:
                degraded_image = (
                    (degraded_image.permute(0, 2, 3, 1) * 127.5 + 128)
                    .clamp(0, 255)
                    .to(torch.uint8)
                    .detach()
                    .cpu()
            ...
            sharp_image, _ = self.G.synthesis(
                w_pivot, torch.zeros_like(q, device=q.device), noise_mode="const", force_fp32=True
            )
            sharp_image = (
                (sharp_image.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).detach().cpu().numpy()[0]
            )
