...
class AdaINResLoss(nn.Module):
    ...
    def __call__(self, x1, y1, x2, y2):
        ...
        content_loss += self.weights[0] * self.criterion(
            x1_vgg["relu1_1"] - y1_vgg["relu1_1"], x2_vgg["relu1_1"] - y2_vgg["relu1_1"]
        )
        content_loss += self.weights[1] * self.criterion(
            x1_vgg["relu2_1"] - y1_vgg["relu2_1"], x2_vgg["relu2_1"] - y2_vgg["relu2_1"]
        )
        content_loss += self.weights[2] * self.criterion(
            x1_vgg["relu3_1"] - y1_vgg["relu3_1"], x2_vgg["relu3_1"] - y2_vgg["relu3_1"]
        )
        content_loss += self.weights[3] * self.criterion(
            x1_vgg["relu4_1"] - y1_vgg["relu4_1"], x2_vgg["relu4_1"] - y2_vgg["relu4_1"]
        )
        content_loss += self.weights[4] * self.criterion(
            x1_vgg["relu5_1"] - y1_vgg["relu5_1"], x2_vgg["relu5_1"] - y2_vgg["relu5_1"]
        )
...
class StatsLoss(torch.nn.Module):
    def forward(self, generated_sharp_images, org_sharp_images, generated_deg_images, org_deg_images):
        mu_ori, std_ori = (org_deg_images - org_sharp_images).mean(dim=[0, 2, 3]), (
            org_deg_images - org_sharp_images
        ).std(dim=[0, 2, 3])
        mu_gen, std_gen = (generated_deg_images - generated_sharp_images).mean(dim=[0, 2, 3]), (
            generated_deg_images - generated_sharp_images
        ).std(dim=[0, 2, 3])
...
class TVLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        width = x.size()[3]
        tv_h = torch.div(
            torch.sum(torch.abs(x[:, :, 1:, :] - x[:, :, :-1, :])), (x.size()[0] * x.size()[1] * (height - 1) * width)
        )
        tv_w = torch.div(
            torch.sum(torch.abs(x[:, :, :, 1:] - x[:, :, :, :-1])),
            (x.size()[0] * x.size()[1] * (height) * (width - 1)),
        )
        return tv_w + tv_h
