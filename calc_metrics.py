...
def subprocess_fn(rank, args, temp_dir):
    ...
    if rank == 0 and args.verbose:
        ...
        q = torch.randn([1, G.q_dim], device=device)
        misc.print_module_summary(G, [z, c, q])
...
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose):
    ...
    # Validate arguments.
    args = dnnlib.util.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose)
    ...
    if data is not None:
        args.dataset_kwargs = dnnlib.util.EasyDict(class_name="training.dataset.ImageFolderDataset", path=data)
    elif network_dict["training_set_kwargs"] is not None:
        args.dataset_kwargs = dnnlib.util.EasyDict(network_dict["training_set_kwargs"])
    else:
