...
class Loss:
    def accumulate_gradients(
        self, phase, real_img, real_degraded_img, real_c, gen_z, gen_c, sync, gain
    ...
    def accumulate_gradients(self, phase, real_img, real_degraded_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function("Gmain_forward"):
                ...
                gen_teacher_img, gen_teacher_features = self.run_Gteacher_synthesis(
                    _gen_ws, sync=(sync and not do_Gpl)
                )
                ...
                loss_Gmain = (
                    torch.nn.functional.softplus(-gen_logits)
                    + torch.nn.functional.softplus(-gen_degraded_logits)
                    + kd_logits * 3
            ...
            with torch.autograd.profiler.record_function("Gpl_forward"):
                ...
                with torch.autograd.profiler.record_function("pl_grads"), conv2d_gradfix.no_weight_gradients():
                    pl_grads = torch.autograd.grad(
                        outputs=[(gen_img * pl_noise).sum()], inputs=[gen_ws, q], create_graph=True, only_inputs=True
                    )[0]
                ...
                pl_degraded_noise = torch.randn_like(gen_degraded_img) / np.sqrt(
                    gen_degraded_img.shape[2] * gen_degraded_img.shape[3]
                )
                with torch.autograd.profiler.record_function(
                    "pl_degraded_grads"
                ), conv2d_gradfix.no_weight_gradients():
                    pl_degraded_grads = torch.autograd.grad(
                        outputs=[(gen_degraded_img * pl_degraded_noise).sum()],
                        inputs=[gen_ws, q],
                        create_graph=True,
                        only_inputs=True,
                    )[0]
        ...
        if do_D_degraded_main:
            with torch.autograd.profiler.record_function("D_degraded_gen_forward"):
                ...
                gen_degraded_logits = self.run_D_degraded(
                    gen_degraded_img, gen_c, sync=False
                )  # Gets synced by loss_Dreal.
                training_stats.report("Loss/scores/fake_degraded", gen_degraded_logits)
                training_stats.report("Loss/signs/fake_degraded", gen_degraded_logits.sign())
                loss_Dgen_degraded = (
                    torch.nn.functional.softplus(gen_degraded_logits) * 2
        ...
        if do_D_degraded_main or do_D_degraded_r1:
            name = (
                "Dreal_Dr1_degraded"
                if do_D_degraded_main and do_D_degraded_r1
                else "Dreal_degraded"
                if do_D_degraded_main
                else "Dr1_degraded"
            )
            with torch.autograd.profiler.record_function(name + "_forward"):
                ...
                if do_D_degraded_main:
                    loss_Dreal_degraded = torch.nn.functional.softplus(
                        -real_degraded_logits
                ...
                if do_D_degraded_r1:
                    with torch.autograd.profiler.record_function("r1_grads"), conv2d_gradfix.no_weight_gradients():
                        r1_degraded_grads = torch.autograd.grad(
                            outputs=[real_degraded_logits.sum()],
                            inputs=[real_degraded_img_tmp],
                            create_graph=True,
                            only_inputs=True,
                        )[0]
