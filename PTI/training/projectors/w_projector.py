...
def project(
    ...
    q_opt = q.clone().detach().float().requires_grad_(True)
    optimizer = torch.optim.Adam([w_opt] + list(noise_bufs.values()), betas=(0.9, 0.999),
                                 lr=hyperparameters.first_inv_lr)

    q_optimizer = torch.optim.Adam([q_opt], betas=(0.9, 0.999),
                                 lr=hyperparameters.q_inv_lr)
    ...
    for step in tqdm(range(num_steps)):
        ...
        synth_images, _ = G.synthesis(ws, q_opt, noise_mode='const', force_fp32=True)
        ...
        if float(loss) <= hyperparameters.w_opt_loss:
            break
    ...
    del G
    return w_opt.repeat([1, num_ws, 1]), q_opt
